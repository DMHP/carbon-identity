...
public class DefaultRequestCoordinator implements RequestCoordinator {
    ...
    public void handle(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            if (request.getParameter("type") != null) {
                // Retrieve AuthenticationRequestCache Entry which is stored from servlet.
                if (sessionDataKey != null) {
                    ...
                    if (authRequest == null) {
                        ...
                        if (log.isDebugEnabled()) {
                            log.debug("No AuthenticationRequest found in the cache for SessionDataKey : "
                                    + sessionDataKey);
                        }

                        if (isCommonAuthLogoutRequest(request)) {
                            if (log.isDebugEnabled()) {
                                log.debug("Ignoring the invalid SessionDataKey: "
                                        + sessionDataKey + " in the CommonAuthLogout request.");
                            }
                        } else {
     ...
     * @throws FrameworkException
     */
    private boolean isUserAllowedToLogin(AuthenticatedUser user) throws FrameworkException {
        ...
        try {
            ...
            throw new FrameworkException("Error while checking existence of user:  " + user.getUserName(), e);
        }
        return false;
    }
