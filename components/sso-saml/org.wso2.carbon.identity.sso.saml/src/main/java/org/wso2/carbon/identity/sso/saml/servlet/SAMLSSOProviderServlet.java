...
public class SAMLSSOProviderServlet extends HttpServlet {
    ...
    private void sendNotification(String errorResp, String status, String message,
        ...
        String queryAppendedUrl = SAMLSSOUtil.appendQueryParamsStringToUrl(redirectURL, queryParams);
    ...
    private void handleAuthenticationReponseFromFramework(HttpServletRequest req, HttpServletResponse resp,
        ...
        if (authResult == null || !authResult.isAuthenticated()) {
                ...
                if (SAMLSSOUtil.validateACS(sessionDTO.getTenantDomain(), sessionDTO.getIssuer(), reqValidationDTO
                    ...
                    String errorResp = SAMLSSOUtil.buildCompressedErrorResponse(reqValidationDTO.getId(),
                                                                                statusCodes, "User authentication " +
                                                                                             "failed", destination);
                    String acsUrl = reqValidationDTO.getAssertionConsumerURL();
            ...
            if (authRespDTO.isSessionEstablished()) { // authenticated
                ...
                String acsUrl = authRespDTO.getAssertionConsumerURL();
                if (StringUtils.isEmpty(acsUrl) && sessionDTO.getIssuer() != null) {
                    SAMLSSOServiceProviderDO serviceProviderDO = getSPConfig(SAMLSSOUtil
                            .getTenantDomainFromThreadLocal(), sessionDTO.getIssuer());
                    if (serviceProviderDO != null) {
        ...
        if (object != null) {
            ...
            if (status == AuthenticatorFlowStatus.INCOMPLETE) {
                if (responseWrapper.isRedirect()) {
                    ...
                    if (responseWrapper.getBufferSize() > 0) {
    ...
    private void sendRequestToFramework(HttpServletRequest request, HttpServletResponse response, String sessionDataKey,
        ...
        requestWrapper.setParameter(FrameworkConstants.SESSION_DATA_KEY, URLEncoder.encode(sessionDataKey,
                FrameworkUtils.UTF_8));
